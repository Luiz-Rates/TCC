    ...
    <div class="mx-auto max-w-6xl rounded-3xl border border-slate-800/70 bg-slate-950/90 p-6 text-slate-100 shadow-2xl shadow-slate-900/70 backdrop-blur sm:p-8">
        <div class="flex flex-col gap-2 border-b border-slate-800/70 pb-6 text-center sm:text-left">
            ...
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <h2 class="text-3xl font-bold text-white">Lista de Clientes</h2>
                <a href="{{ route('clientes.create') }}"
                    ...
                    <span class="text-lg">+</span> Novo Cliente
                </a>
            </div>
            <p class="text-sm text-slate-400">Veja rapidamente os dados de contato dos clientes e mantenha o cadastro sempre organizado.</p>
        </div>
        ...
        <form method="GET" action="{{ route('clientes.index') }}"
            ...
            <div class="flex-1">
                <label for="search" class="text-xs font-semibold uppercase tracking-wide text-slate-400">Buscar cliente</label>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex flex-wrap items-center justify-center gap-3 text-sm font-semibold uppercase tracking-wide sm:justify-end">
                                    <a href="{{ route('clientes.show', $cliente) }}"
                                        class="rounded-xl border border-slate-600/40 px-3 py-2 text-slate-200 transition hover:border-blue-400 hover:bg-blue-500/10">
                                        Perfil
                                    </a>
